...
using STFT_ =
    STFT<FFT_SIZE, WIN_SIZE, STFTWindowType::HanningSqrt, STFTOverlapAmt::Half>;
...
class STFTTest : public ::testing::Test {
 protected:
  ...
  void set_random_input() {
    white_noise.process(input.get_write_ptr(), input.get_len(), 1);
    white_noise.process(input_stft.get_write_ptr(), input_stft.get_len(), 1);
  }

  float eps = 1e-5f;

  AudioBuffer<FFT_SIZE> input;
  AudioBuffer<FFT_SIZE> output;
  AudioBuffer<2 * N_FREQS> tmp_buff;
  AudioBuffer<N_FRAMES * HOPSIZE> input_stft;
  AudioBuffer<N_FRAMES * HOPSIZE> output_stft;
  AudioBuffer<N_FREQS, 1, std::complex<real_type>> cpx_buff;
  WhiteNoiseGen white_noise;
  STFT_ stft;
};

TEST_F(STFTTest, ComputeFFTReordering) {
  stft.compute_fft(input.get_read_ptr(), output.get_write_ptr());
  // DC
...
TEST_F(STFTTest, TestFFTInversion) {
  ...
  stft.compute_fft(x, tmp);
  stft.compute_ifft(tmp, y);
...
TEST_F(STFTTest, TestSTFTInversion) {
  ...
  expect_all_near(x, y + HOPSIZE, input_stft.get_len() - 4 * HOPSIZE, eps);
}
