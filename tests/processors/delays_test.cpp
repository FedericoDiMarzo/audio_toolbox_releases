...
TEST_F(DelaysTest, IntegerDelayProcess) {
  auto* x = input_buffer.get_read_ptr();
  auto* y = output_buffers[0].get_read_ptr();
  auto zeros = get_zeros<real_type, INT_DLY>();
  expect_all_near(zeros.data(), y, INT_DLY);
  expect_all_near(x, y + INT_DLY, input_buffer.size() - INT_DLY);
}
...
TEST_F(DelaysTest, IntegerDelayReset) {
  ...
  for (uint32_t i = 0; i < INT_DLY; i++) {
    real_type y = delay_int.process_sample(x[i]);
...
TEST_F(DelaysTest, FractionalDelayProcess) {
...
TEST_F(DelaysTest, FractionalDelayProcessInteger) {
  delay_frac.set_delay(INT_DLY);
  delay_frac.reset();
  EXPECT_NEAR(delay_frac.get_delay(), INT_DLY, 1e-6);
  auto* x = input_buffer.get_read_ptr();
  auto* y = output_buffers[1].get_write_ptr();
  delay_frac.process(x, y, input_buffer.size());
  expect_all_near(x, y + INT_DLY, input_buffer.size() - INT_DLY);
}
...
TEST_F(DelaysTest, ModulatedFractionalDelayProcessInteger) {
  delay_frac.set_delay(INT_DLY);
  delay_frac.reset();
  EXPECT_NEAR(delay_frac.get_delay(), INT_DLY, 1e-6);
  auto* x = input_buffer.get_read_ptr();
