#include "audio_toolbox/static/seekable_audio_buffer_static.hpp"
...
class SeekableAudioBufferStaticTest : public ::testing::Test
{
  protected:
    void SetUp() override
    {
    ...
    SeekableAudioBufferStatic<BUFF_LEN, BLOCK_LEN> mono_buff;
    std::array<real_type, BLOCK_LEN> block_buff;
};

TEST_F(SeekableAudioBufferStaticTest, Constructor)
{
...
TEST_F(SeekableAudioBufferStaticTest, Update)
{
...
TEST_F(SeekableAudioBufferStaticTest, Reset)
{
    auto* ptr_0 = mono_buff.get_seek_ptr();
    mono_buff.update();
    mono_buff.reset();
...
TEST_F(SeekableAudioBufferStaticTest, CopyBlockTo)
{
    mono_buff.update();
