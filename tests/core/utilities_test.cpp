...
TEST_F(UtilitiesTest, UpdateDispatcher) {
  bool none_updated =
      std::all_of(std::begin(updatables), std::end(updatables),
                  [](const Updatable& x) { return !x.updated; });
  EXPECT_TRUE(none_updated);
  dispatcher.update();
  bool all_updated = std::all_of(std::begin(updatables), std::end(updatables),
                                 [](const Updatable& x) { return x.updated; });
