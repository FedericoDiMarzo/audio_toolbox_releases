...
class MathTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    stft.compute_fft(input.get_read_ptr(), input_fft.get_write_ptr());
  }

  void TearDown() override {}

  WhiteNoiseGen white_noise_gen;
  AudioBuffer<N_FFT> input;
  AudioBuffer<2 * N_FREQS> input_fft;
  STFT<N_FFT> stft;
};
...
// TEST_F(MathTest, EnergyFreqDomain) {
//   // They should be the same due to Parseval's theorem
//   auto energy_td = compute_energy(input.get_read_ptr(), input.get_len());
//   auto energy_fd = compute_energy_freq_domain(input_fft.get_read_ptr(),
//                                               N_FREQS);
//   print_array(input.get_read_ptr(), input.get_len());
//   print_array(input_fft.get_read_ptr(), input_fft.get_len());
//   EXPECT_NEAR(energy_td, energy_fd, 1e-8);
// }
...
TEST_F(MathTest, ComputeIntRange) {
  auto range = compute_int_range<10>(20);
