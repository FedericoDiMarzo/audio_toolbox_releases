...
class MidiTest : public ::testing::Test {
 protected:
  ...
  std::vector<uint8_t> midi_msg = {status_byte,    data_byte, data_byte,
                                   cc_status_byte, cc_number, cc_value};
...
TEST_F(MidiTest, MidiCCParameterGetStart) { EXPECT_EQ(cc_param.get(), 0); }
