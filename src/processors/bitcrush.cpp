...
audio_toolbox::BitCrush::BitCrush(uint8_t bit_depth, uint32_t downsample_factor,
...
void audio_toolbox::BitCrush::reset() noexcept {
  downsample_count = 0;
  last_sample = 0;
  next_sample = 0;
}

void audio_toolbox::BitCrush::set_bit_depth(uint8_t bit_depth) noexcept {
  this->bit_depth = bit_depth > 23 ? 23 : bit_depth;
}

void audio_toolbox::BitCrush::set_downsample_factor(
    uint32_t downsample_factor) noexcept {
  this->downsample_factor = downsample_factor;
}

void audio_toolbox::BitCrush::set_clip_threshold(
    real_type clip_threshold) noexcept {
  this->clip_threshold = clip_threshold;
}

real_type audio_toolbox::BitCrush::process_sample(real_type input) noexcept {
  /*
  ...
  // Clipping
  u.f = std::max(-clip_threshold, std::min(clip_threshold, input));
  ...
  u.f = last_sample + (next_sample - last_sample) *
                          ((float)downsample_count / downsample_factor);
