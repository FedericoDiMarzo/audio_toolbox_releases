...
real_type compute_A(real_type gain_db)
{
    return std::pow(10, gain_db / 40);
}
...
audio_toolbox::Biquad::Biquad(real_type b0, real_type b1, real_type b2, real_type a1, real_type a2)
{
    set(b0, b1, b2, a1, a2);
}

audio_toolbox::Biquad::Biquad(real_type b0, real_type b1, real_type b2, real_type a0, real_type a1, real_type a2)
{
    set(b0 / a0, b1 / a0, b2 / a0, a1 / a0, a2 / a0);
}

audio_toolbox::Biquad::Biquad(BiquadFilterType type,
...
void audio_toolbox::Biquad::set(real_type b0, real_type b1, real_type b2, real_type a1, real_type a2)
{
    B[0] = b0;
    B[1] = b1;
    B[2] = b2;
...
void audio_toolbox::Biquad::process_sample(const real_type* input, real_type* output) noexcept
{
...
void audio_toolbox::Biquad::design_lowshelf(real_type A, real_type alpha, real_type cs)
{
...
void audio_toolbox::Biquad::design_highshelf(real_type A, real_type alpha, real_type cs)
{
    real_type b0 = A * ((A + 1) + (A - 1) * cs + 2 * compute_sqrt(A) * alpha);
    real_type b1 = -2 * A * ((A - 1) + (A + 1) * cs);
    real_type b2 = A * ((A + 1) + (A - 1) * cs - 2 * compute_sqrt(A) * alpha);
...
void audio_toolbox::Biquad::design_peak(real_type A, real_type alpha, real_type cs)
{
