...
AudioParameter::AudioParameter(real_type value, uint32_t sample_rate, uint32_t block_size, float transition_ms)
    : sample_rate(sample_rate),
      block_size(block_size),
      target_value(value),
      current_value(value)
{
    if ((transition_ms / 1000.0f) < (static_cast<real_type>(block_size) / static_cast<real_type>(sample_rate)))
    {
...
void AudioParameter::set(real_type value) noexcept
{
    target_value = value;
}

real_type AudioParameter::get() const noexcept
{
...
{
    alpha = alpha_from_time_const(transition_ms / 1000.0f,
