...
MidiCCParameter::MidiCCParameter(uint8_t cc_number, real_type min,
                                 real_type max)
    : min(min), max(max), cc_number(cc_number), new_value(0), last_value(0) {};
...
void MidiCCParameter::update(C& midi_tokens) {
  ...
  auto itr = std::find_if(
      std::rbegin(midi_tokens), std::rend(midi_tokens),
