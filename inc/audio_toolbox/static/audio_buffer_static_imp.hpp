...
_AUDIO_BUFFER_TYPE::AudioBufferStatic()
{
...
_AUDIO_BUFFER_TEMPLATE
T* _AUDIO_BUFFER_TYPE::begin() noexcept
{
    return buffer;
}

_AUDIO_BUFFER_TEMPLATE
const T* _AUDIO_BUFFER_TYPE::begin() const noexcept
{
    return buffer;
}

_AUDIO_BUFFER_TEMPLATE
T* _AUDIO_BUFFER_TYPE::end() noexcept
{
...
_AUDIO_BUFFER_TEMPLATE
const T* _AUDIO_BUFFER_TYPE::get_read_ptr() const noexcept
{
    return buffer;
}

_AUDIO_BUFFER_TEMPLATE
T* _AUDIO_BUFFER_TYPE::get_write_ptr() const noexcept
{
    return buffer;
}

_AUDIO_BUFFER_TEMPLATE
uint32_t _AUDIO_BUFFER_TYPE::get_len() const noexcept
{
    return LEN;
}

_AUDIO_BUFFER_TEMPLATE
uint32_t _AUDIO_BUFFER_TYPE::get_channels() const noexcept
{
    return CHANNELS;
}
...
_AUDIO_BUFFER_TEMPLATE
void _AUDIO_BUFFER_TYPE::copy_from_channel(AudioBufferStatic<LEN, 1, T>& mono_buffer, uint32_t channel) noexcept
{
...
_AUDIO_BUFFER_TEMPLATE
void _AUDIO_BUFFER_TYPE::copy_to_channel(AudioBufferStatic<LEN, 1, T>& mono_buffer, uint32_t channel) noexcept
{
    for (uint32_t i = 0; i < LEN; i++)
    {
...
{
    static_assert(sizeof...(Buffers) == CHANNELS, "Number of buffers must match number of channels");
    std::array<AudioBufferStatic<LEN, 1, T>*, CHANNELS> buffer_array = { &buffers... };
    for (uint32_t i = 0; i < LEN; i++)
    {
...
void audio_buffer_to_complex(AudioBufferStatic<2 * LEN, CHANNELS, real_type>& interleaved_buffer,
...
template <uint32_t CHANNELS, uint32_t LEN>
void audio_buffer_from_complex(AudioBufferStatic<2 * LEN, CHANNELS, real_type>& interleaved_buffer,
                               AudioBufferStatic<LEN, CHANNELS, std::complex<real_type>>& complex_buffer) noexcept
{
