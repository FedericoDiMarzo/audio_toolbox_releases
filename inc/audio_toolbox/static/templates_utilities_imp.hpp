...
{
    return std::distance(container.begin(), std::find(std::begin(container), std::end(container), value));
};
...
bool remove_item(C& container, T item)
{
    ...
    container.erase(std::remove(std::begin(container), std::end(container), item), container.end());
...
{
    std::visit([&](auto& filter) { filter.process(std::forward<Args>(args)...); }, variant);
}
...
void UpdateDispatcher<T1>::update(Args&&... args)
{
    ...
    std::for_each(std::begin(observers),
                  std::end(observers),
                  [&](T1* observer) { observer->update(std::forward<Args>(args)...); });
}
